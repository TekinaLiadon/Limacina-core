...
2. Скопировать содержимое сборки (dist) в `public/panel/`.

Маршрут `/panel` регистрируется на старте сервера всегда (каталог создаётся при отсутствии), поэтому панель можно деплоить без остановки сервера: копируете файлы — SPA сразу отдаётся.

Панель общается с API этого же сервера (`/v1/common/auth/*`, `/v1/panel/*`), дополнительный CORS не нужен. По умолчанию кросс-доменные запросы запрещены (CORS закрыт); если панель хостится на отдельном домене — перечислите его в `CORS_ORIGINS`.
